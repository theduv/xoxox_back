...
const server = https.createServer({key: fs.readFileSync('./back.xoxox.dev/back.xoxox.dev.key'), cert: fs.readFileSync('./back.xoxox.dev/fullchain.cer')}, app);
...
const onPlayerJoinRoom = (data, socket) => {
	...
	room.numPlayers > 2 ? room.spectators.push(data.username) : room.spectators.push(data.username)
	room.chat.push({
		username: "",
